...
export function RegisterForm() {
  ...
  const form = useForm<RegisterValues>({
    ...
    mode: "onBlur",
    defaultValues: { fullName: "", email: "", invitationCode: "", password: "", confirmPassword: "" },
  });
  const { errors, isSubmitting } = form.formState;

  async function onSubmit(values: RegisterValues) {
    try {
      await registerUser(values.email, values.password, values.fullName, values.invitationCode);
      ...
      } else if (errorMessage.toLowerCase().includes("invitation")) {
        form.setError("invitationCode", { message: errorMessage });
      } else {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} noValidate className="space-y-[18px]">
        ...
        <FieldBlock label="Invitation code" htmlFor="invitationCode" error={errors.invitationCode?.message} hint="Required">
          <input
            ...
            className="auth-input font-mono uppercase tracking-[0.05em]"
