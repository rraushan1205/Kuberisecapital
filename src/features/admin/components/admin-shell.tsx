...
const navigation = [
  ...
  { href: "/admin/logs", label: "Execution logs", icon: UserRoundCheck },
...
function AdminHeader({ onOpenNavigation }: { onOpenNavigation: () => void }) {
  ...
  async function logout() {
    ...
    try {
      ...
      // adminApi.logout already clears sessionStorage in its finally block.
    } finally {
      // Tab-scoped sessionStorage is cleared by adminApi.logout (belt-and-braces here).
      clearSession("admin");
